...
public class SearchBFS extends JPanel {
	...
	public SearchBFS(JPanel cardPanel, MyView myview) {
		setLayout(null);
		...
		btnBack.setIcon(new ImageIcon(SearchBFS.class.getResource("/com/hit/resources/back.png")));
		...
		btnSearch.setIcon(new ImageIcon(SearchBFS.class.getResource("/com/hit/resources/search.png")));
		...
		lblTotalFlights.setBounds(55, 161, 219, 35);
		lblTotalFlights.setVisible(false);
		add(lblTotalFlights);
	...
	/*
	 * Function to show up the result of the path user search, its show the quantity of flights and the flights themselves
	 */
	public void showResult(LinkedList<String> path) {
		StringBuilder flights = new StringBuilder();
		int count = 0; // count flights
		for(int i=0;i<path.size()-1;i++)
		{
			flights.append(path.get(i)+" -> "+path.get(i+1)+"\n");
			count++;
		}
		textArea.setText(flights.toString());
		
		lblTotalFlights.setText("Number Of Flights: "+ count);
